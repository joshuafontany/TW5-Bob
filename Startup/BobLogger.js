  ...
  if($tw.node) {
    ...
    */
    process.on('unhandledRejection', function(err) {
      ...
      let abort = $tw.Bob.settings.logger.abortUnhandledRejections || false;
      if(abort) {
        process.abort()
      }
      process.exit(1)
    ...
    /*
      Set up file logging
    */
    if($tw.Bob.settings.logger.useFileLogging === 'yes') {
      // Output gets written to a file
      const basePath = $tw.ServerSide.getBasePath();
      const outputFolder = $tw.Bob.settings.logger.outputFolder || './logs';
      const outputBaseFileName = $tw.Bob.settings.logger.outputBaseFileName || 'Log';
      ...
      if($tw.Bob.settings.logger.useSeparateErrorFile === 'yes') {
        const outputErrorFileName = $tw.Bob.settings.logger.outputErrorFileName || 'Error';
      ...
      const ignoreErrors = $tw.Bob.settings.logger.ignoreErrors === 'no'?false:true;
    ...
    if($tw.Bob.settings.logger.useBrowserLogging === 'yes') {
      // TODO this!!
    ...
    if($tw.Bob.settings.logger.useConsoleLogging !== 'no') {
    ...
    $tw.Bob.logger.log = function(/* args */) {
      ...
      if($tw.Bob.settings.logger.useFileLogging === 'yes') {
        $tw.Bob.settings.logger.fileLogLevel = $tw.Bob.settings.logger.fileLogLevel || 2;
        // Output gets written to a file
        if(typeof params.level === 'undefined' || $tw.Bob.settings.logger.fileLogLevel >= params.level) {
          $tw.Bob.logger.file.log(argumentList.join(' '));
        }
      }
      if($tw.Bob.settings.logger.useBrowserLogging === 'yes') {
        $tw.Bob.settings.logger.browserLogLevel = $tw.Bob.settings.logger.browserLogLevel || 2;
        // Output gets written to a tiddler so it is visible in the browser
        if(typeof params.level === 'undefined' || $tw.Bob.settings.logger.browserLogLevel >= params.level) {
          $tw.Bob.logger.browser.log(argumentList.join(' '));
        }
      }
      if($tw.Bob.settings.logger.useConsoleLogging !== 'no') {
        $tw.Bob.settings.logger.consoleLogLevel = $tw.Bob.settings.logger.consoleLogLevel || 2;
        // If another option isn't set than output is logged to the console
        if(typeof params.level === 'undefined' || $tw.Bob.settings.logger.consoleLogLevel >= params.level) {
