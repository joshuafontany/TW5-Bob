...
both node and browser so we are stashing these in the $tw.utils namespace.
...
*/
exports.getCookie = function(cookie, cname) {
  cookie = cookie || ""
  const name = cname + "=";
...
*/
exports.TiddlerHasChanged = function(tiddler, otherTiddler) {
  if(!otherTiddler) {
    return true;
  }
  if(!tiddler) {
  ...
  const hash1 = tiddler.hash || $tw.utils.getTiddlerHash(tiddler);
  const hash2 = otherTiddler.hash || $tw.utils.getTiddlerHash(otherTiddler);
  return hash1 !== hash2;
};
...
/*
  ...
  anything like that.
*/
exports.getTiddlerHash = function(tiddler) {
  const tiddlerString = $tw.utils.stableStringify($tw.utils.normalizeTiddler(tiddler))
  let hash = 0;
...
exports.stableStringify = function(data,opts) {
  if(!opts) opts = {};
...
*/
exports.normalizeTiddler = function(tiddler) {
  let newTid = {};
  if(tiddler) {
    if(tiddler.fields) {
...
*/
exports.createMessageData = function(message) {
  //const id = message.id || makeId();
  const id = $tw.utils.makeMessageId()
  message.id = id;
  ...
  if(['saveTiddler', 'deleteTiddler', 'editingTiddler', 'cancelEditingTiddler'].indexOf(message.type) !== -1) {
    ...
    message.tiddler.hash = $tw.utils.getTiddlerHash(message.tiddler);
  }
  let messageData = {
    message: message,
    id: id,
...
/*
  ...
  again in 500ms
*/
 exports.checkMessageQueue = function() {
  ...
  if($tw.Bob.MessageQueue.length > 0) {
    ...
    const oldMessages = $tw.Bob.MessageQueue.filter(function(messageData) {
      if((Date.now() - messageData.time > $tw.Bob.settings.advanced.localMessageQueueTimeout || 500)
    ...
    oldMessages.forEach(function(messageData) {
      ...
      const targetConnections = $tw.node?(messageData.wiki?$tw.Bob.sessions.filter(function(item) {
    ...
    $tw.Bob.messageQueueTimer = setTimeout($tw.utils.checkMessageQueue, $tw.Bob.settings.advanced.localMessageQueueTimeout || 500);
  } else {
    clearTimeout($tw.Bob.messageQueueTimer);
  ...
  if(connection.socket !== undefined) {
    if(!messageData.ack[index] && connection.socket.readyState === 1) {
      ...
      if(messageData.type === 'saveTiddler' && $tw.browser) {
        ...
        }, $tw.Bob.settings.advanced.saveTiddlerDelay || 200);
      } else {
        console.log(`Sending msg ${messageData.message.id}`);
        connection.socket.send(JSON.stringify(messageData.message));
      }
...
*/
exports.removeRedundantMessages = function(messageData, queue) {
...
/*
  ...
  - If the message is a saveTiddler message and there is another saveTiddler
    ...
    message
*/
exports.messageIsEligible = function(messageData, connectionIndex, queue) {
  let send = false;
  ...
  function nextBit() {
    ...
    if(messageData.type === 'saveTiddler') {
      messageData.message.tiddler = $tw.utils.normalizeTiddler(messageData.message.tiddler)
    }
    ...
    if(messageData.message.wiki === $tw.Bob.sessions[connectionIndex].wiki || $tw.browser || !messageData.message.wiki) {
      let ignore = false;
        ...
        if(messageData.type === 'saveTiddler') {
          queue.forEach(function(message, messageIndex) {
            if(message.type === 'saveTiddler' && message.title === messageData.title) {
              if(!$tw.utils.TiddlerHasChanged(messageData.message.tiddler, queue[messageIndex].message.tiddler)) {
                ignore = true;
              }
            }
          })
...
/*
  ...
  $tw.utils.messageIsEligible, if it isn't than nothing is sent but the messageQueue timer is still reset.
...
*/
exports.sendMessage = function(message, connectionIndex, messageData) {
  connectionIndex = connectionIndex || 0;
  messageData = messageData || $tw.utils.createMessageData(message);
  if($tw.Bob.logger){
  ...
  if($tw.utils.messageIsEligible(messageData, connectionIndex, $tw.Bob.MessageQueue)) {
    $tw.Bob.Timers = $tw.Bob.Timers || {};
    if(messageData.message.tiddler) {
      messageData.message.tiddler = $tw.utils.normalizeTiddler(messageData.message.tiddler);
    }
    // Remove any messages made redundant by this message
    $tw.Bob.MessageQueue = $tw.utils.removeRedundantMessages(messageData, $tw.Bob.MessageQueue);
    if($tw.browser) {
    ...
    _sendMessage($tw.Bob.sessions[connectionIndex], messageData)
  }
  clearTimeout($tw.Bob.messageQueueTimer);
  $tw.Bob.messageQueueTimer = setTimeout($tw.utils.checkMessageQueue, $tw.Bob.settings.advanced.localMessageQueueTimeout || 500);
  return messageData;
